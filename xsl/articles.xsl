...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
                    ...
                    <div class="row">
                        <div class="col-12">
                            <h1 class="display-4 text-center mb-4">
                                <i class="fas fa-newspaper me-3"></i>Articles Trouvés </h1>
                            <p class="text-center text-muted mb-5">
                                <xsl:value-of select="count(articles/article)" /> article(s) dans la
        collection </p>
                        </div>
                    </div>
                ...
                <p class="card-text">
                    <xsl:choose>
                        <xsl:when test="string-length(content) > 150">
                            <xsl:value-of select="substring(content, 1, 150)" />... </xsl:when>
                        <xsl:otherwise>
    ...
    <xsl:template name="format-date">
        ...
        <xsl:variable
            name="month" select="substring($date, 6, 2)" />
